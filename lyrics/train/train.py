...
class ASR(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        decoder_outputs, _ = self.modules.decoder(embedded_tokens,
                                                  encoded_signal,
                                                  self.feat_lens)
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if self.is_ctc_active(stage):
            ...
            loss_ctc = self.hparams.ctc_cost(predictions["ctc_logprobs"],
                                             tokens, self.feat_lens,
                                             tokens_lens)
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        # We also write statistics about test data to stdout and to the logfile.
            ...
            with open(self.hparams.predictions_file, 'w') as json_file:
                json.dump(self.predictions, json_file, indent=2)


def tokenize(hparams):
    tokenizer_file = hparams["tokenizer_model_file"]
    if os.path.exists(tokenizer_file) and os.path.isfile(tokenizer_file):
        logger.info("Tokenizer completed in previous run, skipping.")
        return
    hparams["tokenizer"]()
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.provides("words", "tokens_list", "tokens_bos",
                                     "tokens_eos", "tokens")
